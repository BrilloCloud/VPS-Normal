...
"
    ...
    echo "${bold}${lightblue}                    ...aarch64 architecture detected..."
    echo "${bold}${lightblue}          ...THIS MAY TAKE MORE THAN 15 MINUTES BE PATIENT..."
    if [ $LINUX_ISO = "Ubuntu" ]; then
    echo "${redback} THE UBUNTU DISTRO IS NOT WORKING AT THE MOMENT!!"
    exit
    fi
    echo  "${bold}${lightred} Debian/Ubuntu distributions can take more than 15min to finish the installation."
    echo "⚙️  Script Version: ${version_script}"
    echo "Downloading files to start vps"
    mkdir libraries
