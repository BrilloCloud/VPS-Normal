...
${bold}${lightblue}BrilloCloud Normal VPS
...
${bold}${lightred}⛔️ Root run from PRoot-rs, do you know what you are doing?
        "
    fi
    if [ "${PROOT}" = "FakechRoot + FakeRoot" ]; then
        echo "

${bold}${lightred}⛔️ Root executed from FakechRoot + FakeRoot, do you know what you are doing?
${bold}${lightred}⛔️ To use this variable, you have to be using docker: ashu11a/proot:latest
    ...
    echo "${bold}${lightblue}🔎  Identified Architecture: 64x"
    if [ $LINUX_ISO = "Ubuntu" ]; then
...
📌  The following packages will not install: sudo wget hwloc htop nano neofetch ngrok gotty curl
    "
    else
    echo  "${bold}${lightred}⚠️  Debian/Ubuntu distributions can take more than 15min to finish the installation."
    fi
    echo "📥  Downloading files for vps installation"
    if [ -d libraries ]; then
    echo "Libraries folder already exists, skipping..."
    else
    mkdir libraries
    fi
    echo "Disto Installed: $LINUX_ISO" > libraries/distro_installed
